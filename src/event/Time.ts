...
export const periodically = (action: () => void | Promise<void>, intervalSeconds: number = 5): number => {
    invokeAction(action)
...
export const periodicallyWithState = <T>(action: (state: T) => T | Promise<T>, initialState: T, intervalSeconds?: number): number => {
